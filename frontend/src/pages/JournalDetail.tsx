...
const JournalDetail: React.FC = () => {
  ...
  const [expandedVol, setExpandedVol] = useState<number | null>(journal?.volumes[0]?.number ?? null);
  ...
  const totalArticles = journal.volumes.reduce(
    (sum, v) => sum + v.issues.reduce((s, i) => s + i.articles.length, 0), 0
  );
    ...
    <div className="container-custom py-10 space-y-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-4">
          <img src={journal.image} alt={journal.title} className="w-full aspect-[3/2] object-cover border border-border" />
        </div>
        <div className="lg:col-span-8 space-y-4">
          <div>
            <span className="text-[11px] font-semibold text-secondary uppercase tracking-wider">{journal.category}</span>
            <h1 className="text-2xl uppercase tracking-wider mt-1">{journal.title}</h1>
          </div>
          <p className="text-[14px] text-muted leading-relaxed">{journal.description}</p>
      ...
      <div className="space-y-4">
        ...
        {journal.volumes.map((vol) => (
          <div key={vol.number} className="border border-border bg-surface">
            <button
              onClick={() => setExpandedVol(expandedVol === vol.number ? null : vol.number)}
            ...
            >
              ...
              <ChevronDown className={`h-4 w-4 text-muted transition-transform ${expandedVol === vol.number ? 'rotate-180' : ''}`} />
            </button>

            {expandedVol === vol.number && (
                        ...
                        <div key={article.id} className="px-5 py-4 hover:bg-background/50 transition-colors group cursor-pointer">
                          <div className="flex items-start justify-between gap-4">
                            <div className="min-w-0 flex-grow">
                              ...
                              <p className="text-[12px] text-muted mt-1">{article.authors}</p>
                            ...
                            <div className="shrink-0 pt-1 opacity-0 group-hover:opacity-100 transition-opacity">
                              <ExternalLink className="h-3.5 w-3.5 text-muted/40" />
                            </div>
                          </div>
                        </div>
                      ))}
