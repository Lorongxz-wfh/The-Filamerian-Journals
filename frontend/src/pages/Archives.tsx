...
const Archives: React.FC = () => {
  const [expandedJournal, setExpandedJournal] = useState<number | null>(journals[0]?.id ?? null);
  ...
  const totalArticles = journals.reduce(
    (sum, j) => sum + j.volumes.reduce(
      (vs, v) => vs + v.issues.reduce((is, i) => is + i.articles.length, 0), 0
    ), 0
  );
    ...
    <div className="container-custom py-12 space-y-10">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-px bg-border border border-border">
        {[
          ...
          { label: 'Volumes', value: journals.reduce((s, j) => s + j.volumes.length, 0) },
          { label: 'Issues', value: journals.reduce((s, j) => s + j.volumes.reduce((vs, v) => vs + v.issues.length, 0), 0) },
      ...
      <div className="space-y-3">
        {journals.map((journal) => {
          ...
          const articleCount = journal.volumes.reduce(
            (s, v) => s + v.issues.reduce((is, i) => is + i.articles.length, 0), 0
          );
                ...
                <div className="flex items-center gap-4 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <span className="text-[11px] text-muted">
                      {journal.volumes.length} volume{journal.volumes.length > 1 ? 's' : ''} · {articleCount} articles · ISSN {journal.issn}
                    </span>
                  </div>
                </div>
                <div className="flex items-center gap-3 shrink-0 ml-4">
                  <span className="text-[11px] text-muted bg-background px-2 py-0.5">{journal.category}</span>
              ...
              {isOpen && (
                <div className="border-t border-border">
                  {journal.volumes.map((vol) => {
                    const volKey = `${journal.id}-${vol.number}`;
                      ...
                      <div key={vol.id} className="border-b border-border last:border-b-0">
                        ...
                        >
                          <div className="flex items-center gap-3">
                            <span className="text-[13px] font-semibold text-primary">Volume {vol.number}</span>
                            <span className="text-[11px] text-muted">({vol.year})</span>
                            <span className="text-[11px] text-muted/60">
                              — {vol.issues.length} issue{vol.issues.length > 1 ? 's' : ''}, {vol.issues.reduce((s, i) => s + i.articles.length, 0)} articles
                            </span>
                          </div>
                            ...
                            {vol.issues.map((issue) => (
                              <div key={issue.id} className="border-t border-border">
                                <div className="px-6 py-2.5 flex items-center gap-2 bg-background border-b border-border">
                                  ...
                                  <span className="text-[12px] font-semibold text-primary">Issue {issue.number}</span>
                                  <span className="text-[11px] text-muted">— {issue.publishedAt}</span>
                                  <span className="text-[11px] text-muted/50 ml-auto">{issue.articles.length} articles</span>
                                </div>
                                    ...
                                    <div key={article.id} className="px-8 py-3 hover:bg-surface transition-colors group cursor-pointer">
                                      <div className="flex items-start justify-between gap-4">
                                        <div className="min-w-0">
                                          ...
                                          <p className="text-[11px] text-muted mt-0.5 pl-5">{article.authors}</p>
                                          <div className="flex items-center gap-4 mt-1 pl-5 text-[10px] text-muted/50">
                                            <span>pp. {article.pages}</span>
                                            <span>DOI: {article.doi}</span>
                                          </div>
                                        </div>
                                        <ExternalLink className="h-3 w-3 text-muted/20 shrink-0 mt-1 opacity-0 group-hover:opacity-100 transition-opacity" />
                                      </div>
                                    </div>
                                  ))}
                                </div>
