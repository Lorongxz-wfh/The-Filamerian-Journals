          ...
          <div className="mb-12 relative group/carousel">
            ...
            >
              {latestArticles.map((article) => (
                <Link key={article.id} to={`/articles/${article.id}`} className="shrink-0 w-[85vw] sm:w-[340px] lg:w-[380px] flex flex-col border border-border bg-surface p-6 hover:border-primary hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300 snap-start h-[260px] group relative overflow-hidden">
                  ...
                  <h3 className="text-[15px] font-bold text-primary uppercase tracking-wider mb-3 line-clamp-2 transition-colors">
                    {article.title}
                  </h3>
                  <p className="text-[13px] text-muted leading-relaxed line-clamp-3 mb-5 flex-grow transition-colors">
                    {article.abstract || 'No abstract available.'}
                  </p>
                  <div className="flex items-center justify-between text-[11px] text-muted uppercase tracking-wider truncate pt-4 border-t border-border transition-colors">
                    <span className="truncate pr-4">{article.authors?.map((a: any) => a.name).join(', ') || 'Unknown'}</span>
                    <span className="shrink-0 group-hover:translate-x-1.5 group-hover:text-secondary font-bold text-primary transition-all">→</span>
                  </div>
                </Link>
              ))}
            </div>
